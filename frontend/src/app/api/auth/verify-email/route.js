...
export async function POST(request) {
  try {
    const { email, code, token, user_id, full_name, username, title, role } = await request.json()

    console.log('[v0] Email verification attempt for:', email)
    ...
    // Verify email via n8n email webhook
    ...
    console.log('[v0] Calling n8n email webhook for verification')

    const payload = {
      email: String(email).trim().toLowerCase(),
      code: String(code).trim(),
      token: String(token || code || '').trim(),
      action: 'verify_email',
      email_verified: true,
      is_verified: true,
    ...
    if (!n8nRes.ok) {
      console.error('[v0] n8n email verification failed:', n8nRes.status)
    ...
    console.error('[v0] Verify email endpoint error:', err.message, err.stack)
    return NextResponse.json(
      {
